...
dataset = DataSet(start_date="2022-01-01", target="i_m1sum", scale_target=False, scale_variables=False, time_features=False, resample=resample,dynamic_price=True, demand_price=args["demand_price"], feedin_price=args["feedin_price"]).pipeline()
...
for i in range(epochs):
    ...
    loss, values_loss, states, actions, rewards = model.train()
    mean_reward = rewards
    
    loss_list.append(loss)
    value_loss_list.append(values_loss)
...
plot_rewards_loss(rewards_list, loss_list, value_loss_list, updates=batch_size)
